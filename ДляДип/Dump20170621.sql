...
INSERT INTO `test_info` VALUES (1,5,1,'Тест по информационной безопастности'),(3,3,0,'ывф');
...
INSERT INTO `user_result` VALUES (1,2,1,'2017-06-18 06:45:49'),(2,2,2,'2017-06-18 08:49:10'),(3,2,1,'2017-06-21 14:05:11'),(4,2,1,'2017-06-21 14:18:21'),(5,2,1,'2017-06-21 14:27:43'),(6,2,1,'2017-06-21 14:37:47');
...
INSERT INTO `usertestdetails` VALUES (1,6,16),(2,6,16),(3,6,16),(4,6,16),(3,6,17),(2,6,18),(3,6,18),(4,6,18),(3,6,19),(4,6,19),(3,6,20),(2,7,21),(3,7,21),(1,7,22),(3,7,22),(4,7,22),(2,7,23),(3,7,23),(1,7,24),(3,7,24),(4,7,24),(3,7,25),(3,7,26),(1,8,27),(3,8,27),(1,8,28),(2,8,28),(3,8,28),(4,8,28),(3,8,29),(2,8,30),(3,8,30),(4,8,30),(3,9,31),(3,9,32),(1,9,33),(3,9,33),(4,9,33),(1,9,34),(3,9,34),(3,9,35),(3,10,36),(1,10,37),(3,10,37),(4,10,37);
...
--
/*!50003 DROP FUNCTION IF EXISTS `user_answers_count` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `user_answers_count`(idTest bigint(20), idResult bigint(20)) RETURNS int(11)
BEGIN
DECLARE select_var integer;
SET select_var = (select SUM(if(trueAns.true_ans = usAns.true_ans AND usAllAns.all_ans = usAns.true_ans, 1, 0)) as RightAnswers from
	(
		select ID_Q, count(id_a) as true_ans from qs_ar as qa
		Join answer as ans on ans.ID = qa.ID_A 
		where 
		qa.ID_Q in (select tp.ID_question from test_pool as tp where tp.ID_test = idTest)
		and ans.true_variant = 1
		group by ID_Q
    ) as trueAns
Join (
		select ID_questions, count(ID_Answer) as true_ans from usertestdetails as utd
		join answer as ans on ans.id = utd.ID_Answer 
		where ID_user_result = idResult and ans.true_variant = 1
		group by ID_questions
	) as usAns on usAns.ID_questions = trueAns.ID_Q
Join (
		select ID_questions, count(ID_Answer) as all_ans from usertestdetails as utd
		join answer as ans on ans.id = utd.ID_Answer 
		where ID_user_result = idResult
		group by ID_questions
) as usAllAns on usAllAns.ID_questions = trueAns.ID_Q);
RETURN select_var;
END ;;
DELIMITER ;
...
-- Dump completed on 2017-06-21 17:43:37
